...
here deliberately, and in our focused local HTR runs `peam-ssz` still remained
the fastest implementation in the comparison set even with only `sha2`'s
`compress` feature enabled.
...
- The crate enables `sha2`'s `compress` API and currently enables `sha2/asm`
  by default on `aarch64`, `x86_64`, and `x86`.
